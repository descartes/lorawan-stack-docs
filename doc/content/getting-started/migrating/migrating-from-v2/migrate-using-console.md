...
{{< warning >}} For ABP device, you need to completely delete it from {{% ttnv2 %}}, especially if you have not re-programmed it for a new **DevAddr**. Having the same session in {{% ttnv2 %}} and {{% tts %}} would  introduce some serious conflicts. {{</ warning >}}
